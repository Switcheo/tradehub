...
When upgrading a minor binary version (e.g. 1.11.0 to 1.11.1), there should be no changes in consensus or chain state. In this case, we can simply patch the node and CLI binaries - `switcheod` and `switcheocli` by hot replacing them into v1.11.0 folder, which is where cosmovisor/current is pointing to. This should only be done when your chain have been fully caught up.
...
cp -r ~/.switcheod/cosmovisor/upgrades/v1.11.0 ~/.switcheod/cosmovisor/upgrades/v1.11.0.bak

# stop services
switcheoctl stop
# replace switcheod in vx.x.x, cosmovisor/current is pointing to this
sudo cp /etc/switcheoctl/bin/switcheod ~/.switcheod/cosmovisor/upgrades/v1.11.0/bin
# replace switcheocli, cosmovisor/current is pointing to this
sudo cp /etc/switcheoctl/bin/switcheocli ~/.switcheod/cosmovisor/upgrades/v1.11.0/bin
